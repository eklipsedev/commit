...
function portableTextComponents(paragraphClassName?: string): PortableTextComponents {
  const paragraphClass =
    paragraphClassName ?? 'mb-4 break-words text-base leading-relaxed last:mb-0'
  const linkClass = paragraphClassName
    ? 'underline underline-offset-4 decoration-current'
    : 'break-all underline underline-offset-2 [overflow-wrap:anywhere]'
...
type BodyPortableTextProps = {
  ...
  className?: string
  /** Overrides the default paragraph size (legal pages use text-base). */
  paragraphClassName?: string
}

export function BodyPortableText({
  value,
  className,
  paragraphClassName,
